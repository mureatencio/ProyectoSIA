...
namespace SistemaContable.BaseDatos
{
    public static class AccesoProyectos
    {
        public static List<Proyecto> obtenerProyectosActivos(object[] pArgumentos)
        {
            List<Proyecto> resultado = new List<Proyecto>();
            ...
            try
            {
                dataReader = SqlServer.Instance.traerDataReader("obtenerProyectosActivos", pArgumentos);
                if (dataReader.HasRows)
                {
                    while (dataReader.Read())
                    {
